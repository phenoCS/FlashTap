...
| 文件夹 | 路径 | 性质 | bat 文件大小 |
|--------|------|------|:--:|
| **flashtap_V0.01** ⚠️ | `c:\Users\PYX\Desktop\flashtap_V0.01\` | **旧版废弃代码** | 2,308 bytes |
| **flashtap_V0.01 - 副本** ✅ | `c:\Users\PYX\Desktop\flashtap_V0.01 - 副本\` | **当前工作区（改这里）** | 2,912 bytes |
...
dir "c:\Users\PYX\Desktop\flashtap_V0.01 - 副本\一键安装FlashTap.bat"
```
...
| 角色 | 位置 | 说明 |
|------|------|------|
| **开发真源 / 工作区**（改这里） | `c:\Users\PYX\Desktop\flashtap_V0.01 - 副本\` | AI 的工作目录，所有脚本修改在此 |
...
| **VM 登录账户** | `61959` / 密码 `123` | **用户实际使用此账户** |
...
```powershell
robocopy "c:\Users\PYX\Desktop\flashtap_V0.01 - 副本" "C:\flashtap" /MIR /XF *.log /XD __pycache__ models
```
...
REM 2) 看 61959 账户装了哪些 VS Code 扩展（关键：有无 cpptools / 中文包）
cmd /c "dir /b C:\Users\61959\.vscode\extensions"

REM 3) 看 61959 的 locale.json（空=英文界面）
cmd /c "type \"C:\Users\61959\AppData\Roaming\Code\User\locale.json\""
...
| # | 症状 | 根因 | 关键文件/行 |
|---|------|------|------------|
| R1 | F5 用不了（无 C/C++ 调试器） | `ms-vscode.cpptools` 没装进 **61959** 账户（装到了 Administrator 或被网络挡掉）；`extensions.list` 注释说 cpptools "在 WSL 远端单独装"，但 VM **没 WSL** | `extensions.list`、`setup-cpp-env.ps1` 的 `Install-CpptoolsExtension`（用 `Code.exe` 直接装，没重置 env 到 61959） |
| R2 | VS Code 是英文 | 中文包 `ms-ceintl.vscode-language-pack-zh-hans` 在 61959 下，但 `locale.json` 没写/为空 | `install-vscode.ps1` 的 locale 写入逻辑、主脚本启动参数 |
...
| R4 | 提权装错账户 | 安装器 UAC 提权以 Administrator 跑，`code --install-extension` 和写 PATH 都进了 Administrator，而非 61959 | `setup-cpp-env.ps1`、`Setup-FlashTap.ps1` 的提权段 |
...
| `install-vscode.ps1` | VS Code 安装 + 4 个扩展 + locale | 69-75 行重置 `$env:USERPROFILE`/`APPDATA` 到 61959，**这是正确范式**，cpptools/locale 应复用此机制 |
| `extensions.list` | 扩展清单 | 注释说 cpptools 不在此列表（假定 WSL 装），但 VM 无 WSL → 矛盾（R1） |
| `.flashtap-env.txt` | 目标用户信息（61959 等） | `install-vscode.ps1` 从中读用户并重设 env |
...
- [ ] VM 内 `C:\Users\61959\.vscode\extensions` 含 `ms-vscode.cpptools`
- [ ] `C:\Users\61959\AppData\Roaming\Code\User\locale.json` 内容为 `{"locale":"zh-cn"}`
- [ ] 打开 VS Code 为**中文界面**
...
1. 在桌面副本 `flashtap_V0.01 - 副本` 修 R1–R5（重点：让 cpptools + locale.json 走 `install-vscode.ps1` 的 61959 env 重置范式；让主脚本 Windows 分支也带文件夹启动 VS Code；统一 MinGW/Windows 编译路径）。
...
之前的 AI 在 VM 里"找了半天说没安装"——根因是它不知道：VM 名 `flashtap_test`、Z: 共享（`\\VBoxSvr\flashtap` → 宿主 `C:\flashtap`）、guestcontrol 管道、真实产物路径（`C:\FlashTap`、`C:\Users\61959`）。本节把"人点 + AI 查"的完整动作写死，**照做即可，无需重新摸索**。
...
1. 在 VM 内 **Win+R → `cmd` → Ctrl+Shift+Enter 以管理员运行**，执行：
   ...
   ```
   > ⚠️ 权限坑：`61959` 令牌被 UAC 过滤，改不了系统服务（`拒绝访问`）；`Administrator` guestcontrol 又登不进。所以**必须用户在 VM 内手动以管理员运行**，AI 无法自动完成这一步。
...
- `C:\Users\61959\.vscode\extensions` 是否含 `ms-vscode.cpptools`（R1）
- `C:\Users\61959\AppData\Roaming\Code\User\locale.json` 内容（R2）
...
3. 不知道用户实际登录账户是 `61959`，而安装器提权把东西装到了 Administrator；
...
- **情况 A（你在宿主本机前）**：直接在弹出窗口里登录 `61959` / `123`，双击 `Z:\一键安装FlashTap.bat`。最像真机，无黑屏/卡顿。
...
```bat
REM 宿主机算副本哈希
certutil -hashfile "c:\Users\PYX\Desktop\flashtap_V0.01 - 副本\Setup-FlashTap.ps1" SHA256
...
1. VM 已开 VRDE(3390) → 远程桌面连 `localhost:3390`（或 宿主IP:3390），登录 `61959/123`。
